...
def add_watermark(image: np.ndarray, watermark_text: str, position: Tuple[int, int] = (10, 10),
                  font_scale: float = 1.0, color: Tuple[int, int, int] = (255, 255, 255),
                  thickness: int = 2) -> np.ndarray:
    return cv2.putText(image.copy(), watermark_text, position, cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness)
...
def change_format(image: np.ndarray, new_format: str) -> bytes:
    encoded_param = [int(cv2.IMWRITE_JPEG_QUALITY), 90] if new_format.lower() == "jpg" else []
